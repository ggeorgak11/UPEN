...
class Trainer(BaseTrainer):
    ...
    def train_step(self, input_batch, step_count):
        ...
        pred_map_loss_spatial.sum().backward(retain_graph=True)
        self.optimizers_dict['predictor_model'].step()

        pred_maps_spatial = pred_output['pred_maps_spatial']

        output = {}
        output['maps'] = {'pred_maps_spatial':pred_maps_spatial.detach()}
        output['metrics'] = {'pred_map_err_spatial': loss_output['pred_map_err_spatial']}
        output['losses'] = {'pred_map_loss_spatial': pred_map_loss_spatial}
    ...
    def test(self):
        ...
        overall_confusion_matrix_spatial = None
        for tstep, batch in enumerate(tqdm(test_data_loader,
            ...
            with torch.no_grad():
                ...
                if overall_confusion_matrix_spatial is None:
        ...
        output = {}
        output['metrics'] = {'overall_pixel_accuracy_spatial':mAcc_sp,
                             'per_class_pixel_accuracy_spatial':class_mAcc_sp,
                             'mean_interesction_over_union_spatial':mIoU_sp,
                             'mean_f1_score_spatial':mF1_sp}
        output['losses'] = {'pred_map_err_spatial': loss_output['pred_map_err_spatial']}
        output['maps'] = {'pred_maps_spatial':pred_maps_spatial}
